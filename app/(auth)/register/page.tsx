...
import { GraduationCap, Lock, Mail, User, BookOpen, ArrowRight, AlertCircle, CheckCircle2, Clock } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const [yearOfStudy, setYearOfStudy] = useState<string>('4th Year');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // SVCE Email Domain Validation for Students
    if (role === 'student' && !email.toLowerCase().endsWith('@svce.ac.in')) {
      setErrorMsg('Students must register using their official SVCE college email ID ending with @svce.ac.in');
      return;
    }
    ...
    if (res.success) {
      if (role === 'student') {
        setPendingMessage('Registration submitted successfully! Your student account is currently PENDING ADMINISTRATOR APPROVAL. Once approved by the placement admin, you can log in.');
        setTimeout(() => {
          router.push('/login');
        }, 3500);
      } else {
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden selection:bg-blue-600 selection:text-white">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 text-center">
        ...
        <p className="mt-2 text-xs text-slate-400">
          Official SVCE Student & Faculty Assessment Portal
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10 px-4">
        <div className="bg-slate-900 border border-slate-800 py-8 px-6 shadow-2xl sm:rounded-3xl sm:px-10">
          ...
          {errorMsg && (
            <div className="mb-4 p-3.5 bg-red-500/10 border border-red-500/30 rounded-xl text-xs text-red-400 flex items-start space-x-2">
          ...
          {pendingMessage && (
            <div className="mb-4 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl text-xs text-amber-300 flex items-start space-x-3 animate-in fade-in">
              <Clock className="w-5 h-5 text-amber-400 shrink-0 mt-0.5 animate-pulse" />
              <div>
                <strong className="block font-bold text-amber-200 mb-0.5">Account Pending Admin Approval</strong>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1.5">
                {role === 'student' ? 'Official SVCE Email ID (@svce.ac.in)' : 'College Email'}
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={role === 'student' ? 'studentname@svce.ac.in' : 'faculty@svce.ac.in'}
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                />
              </div>
              {role === 'student' && (
                <p className="text-[10px] text-blue-400 mt-1 font-mono">
                  * Must end with @svce.ac.in
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                >
                  ...
                  <option value="Electrical & Elect">Electrical & Elect</option>
                </select>
              </div>
            ...
            >
              {isLoading ? 'Creating Account...' : 'Register Student Profile'}
