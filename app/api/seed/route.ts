...
export async function GET(req: NextRequest) {
  // Security guard: Prohibit unauthenticated production seeding
  const isDev = process.env.NODE_ENV !== 'production';
  const authHeader = req.headers.get('x-admin-secret');
  const secretKey = process.env.ADMIN_SEED_SECRET || 'dev_secret_seed_key';

  if (!isDev && authHeader !== secretKey) {
    return NextResponse.json({ success: false, error: 'Unauthorized: Seed endpoint is restricted.' }, { status: 403 });
  }
