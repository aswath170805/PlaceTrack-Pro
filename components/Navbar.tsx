...
export default function Navbar() {
  ...
  // Show Navbar strictly when inside student, faculty, or admin portal routes
  const isRolePage = 
  ...
  if (!isRolePage || isTestTakingPage) {
    return null;
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
            ...
            {role === 'admin' && (
              <>
                ...
                <Link 
                  ...
                  className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors ${pathname.includes('/live-monitor') ? 'bg-red-50 text-red-600 font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                >
                ...
                <Link 
                  href="/admin/tests" 
                  className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors ${pathname.includes('/tests') ? 'bg-indigo-50 text-indigo-600 font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                >
                ...
                <Link 
                  href="/admin/reports" 
                  className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors ${pathname.includes('/reports') ? 'bg-emerald-50 text-emerald-600 font-bold' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                >
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Active Role Pill Badge: Restricted to static badge for Student and Faculty */}
            {role === 'student' ? (
              <div className="px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider flex items-center space-x-1.5 bg-blue-50 text-blue-700 border border-blue-200 shadow-xs">
              ...
              <div className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center space-x-1.5 shadow-xs border bg-amber-50 text-amber-700 border-amber-200">
              ...
              <div className="relative" ref={profileRef}>
                ...
                >
                  ...
                  <div className="hidden sm:block text-left pr-1">
                    ...
                    <span className="block text-[10px] text-slate-400 capitalize font-medium">{role === 'student' ? 'Student' : role}</span>
                  </div>
                  ...
                  <div className="absolute right-0 mt-2 w-72 bg-white rounded-2xl border border-slate-200 shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                    ...
                    <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 space-y-1.5">
                      ...
                      <div className="flex items-center text-[11px] text-slate-500 truncate">
                        ...
                        <span className="truncate">{user?.email || 'student@svce.ac.in'}</span>
                      </div>

                      <div className="flex items-center text-[11px] text-slate-500">
                        <Building2 className="w-3 h-3 mr-1.5 text-slate-400 shrink-0" />
                        <span>Dept & Year: <strong className="text-slate-700">{user?.department || 'CSE'} ({user?.year_of_study || '4th Year'})</strong></span>
                      </div>
                    </div>

                    {/* Role Switching: Available strictly for Admin users */}
                    {role === 'admin' && (
                      <div className="px-3 py-2 border-b border-slate-100">
                        ...
                        <div className="space-y-1">
                          <button
                            ...
                            className="w-full text-left px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between text-slate-600 hover:bg-slate-50"
                          >
