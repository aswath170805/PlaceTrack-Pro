...
RETURNS TRIGGER AS $$
BEGIN
  -- If the role is NOT service_role, block changes to role and is_verified
  IF auth.role() <> 'service_role' THEN
    IF NEW.role <> OLD.role THEN
      RAISE EXCEPTION 'Unauthorized: Role modifications are not permitted from frontend requests.';
    END IF;
    IF NEW.is_verified <> OLD.is_verified THEN
      RAISE EXCEPTION 'Unauthorized: Verification status modifications are not permitted from frontend requests.';
    END IF;
  END IF;
  RETURN NEW;
END;
