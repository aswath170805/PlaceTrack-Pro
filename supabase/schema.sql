...
ALTER TABLE public.test_attempts ADD COLUMN IF NOT EXISTS risk_score INT DEFAULT 0;
ALTER TABLE public.test_attempts ADD COLUMN IF NOT EXISTS risk_level TEXT DEFAULT 'Normal';
ALTER TABLE public.test_attempts ADD COLUMN IF NOT EXISTS termination_reason TEXT;

-- 13. Proctoring Evidence Table
CREATE TABLE IF NOT EXISTS public.proctoring_evidence (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  attempt_id UUID REFERENCES public.test_attempts(id) ON DELETE CASCADE,
  student_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,
  event_id UUID,
...
-- 14. Gamification: Student XP Table
...
-- 15. Gamification: Student Streaks Table
...
-- 16. Gamification: Student Badges Table
...
-- 17. Topic Performance Statistics Table
...
-- 18. Practice Recommendations Table
...
-- 19. Placement Readiness Scores Table
CREATE TABLE IF NOT EXISTS public.student_readiness_scores (
  student_id UUID PRIMARY KEY REFERENCES public.profiles(id) ON DELETE CASCADE,
  overall_score NUMERIC DEFAULT 75,
  aptitude_score NUMERIC DEFAULT 75,
  logical_score NUMERIC DEFAULT 75,
  programming_score NUMERIC DEFAULT 75,
  technical_score NUMERIC DEFAULT 75,
  communication_score NUMERIC DEFAULT 75,
  interview_score NUMERIC DEFAULT 75,
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Enable RLS on new tables
ALTER TABLE public.proctoring_evidence ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.student_readiness_scores ENABLE ROW LEVEL SECURITY;

-- RLS Policies for Gamification & Readiness
CREATE POLICY "Students view own XP" ON public.student_xp FOR SELECT USING (student_id = auth.uid() OR public.get_user_role() IN ('faculty', 'admin'));
CREATE POLICY "Students view own streaks" ON public.student_streaks FOR SELECT USING (student_id = auth.uid() OR public.get_user_role() IN ('faculty', 'admin'));
CREATE POLICY "Students view own badges" ON public.student_badges FOR SELECT USING (student_id = auth.uid() OR public.get_user_role() IN ('faculty', 'admin'));
CREATE POLICY "Students view own topic stats" ON public.student_topic_stats FOR SELECT USING (student_id = auth.uid() OR public.get_user_role() IN ('faculty', 'admin'));
CREATE POLICY "Students view own recommendations" ON public.practice_recommendations FOR SELECT USING (student_id = auth.uid() OR public.get_user_role() IN ('faculty', 'admin'));
CREATE POLICY "Students view own readiness scores" ON public.student_readiness_scores FOR SELECT USING (student_id = auth.uid() OR public.get_user_role() IN ('faculty', 'admin'));
CREATE POLICY "View proctoring evidence" ON public.proctoring_evidence FOR SELECT USING (student_id = auth.uid() OR public.get_user_role() IN ('faculty', 'admin'));
