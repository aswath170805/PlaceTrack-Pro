...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function loadUserSession() {
      ...
      const savedRole = typeof window !== 'undefined' ? (localStorage.getItem('placetrack_role') as 'student' | 'faculty' | 'admin' | null) : null;
      if (savedRole) {
    ...
    try {
      ...
      if (data.user) {
        ...
        if (profile) {
          setUser(profile as Profile);
          setRole(profile.role as any);
          if (typeof window !== 'undefined') localStorage.setItem('placetrack_role', profile.role);
        }
      }
      setIsLoading(false);
      return { success: true };
  ...
  const signUpWithEmail = async (data: { email: string; pass: string; fullName: string; role: 'student' | 'faculty'; department: string; yearOfStudy?: string; batchId?: string }) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') localStorage.setItem('placetrack_role', newProfile.role);
      setIsLoading(false);
      return { success: true };
    } catch (e: any) {
  ...
  const logout = async () => {
    ...
    setUser(null);
    setRole(null);
    if (typeof window !== 'undefined') localStorage.removeItem('placetrack_role');
  };
